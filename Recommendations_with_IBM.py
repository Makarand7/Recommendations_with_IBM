...
def user_user_recs_part2(user_id, m=10):
    ...
    for _, row in sorted_neighbors.iterrows():
        ...
        recommended.update(unseen)
        if len(recommended) >= m:
            break

    # Sort the recommendations to ensure consistent ordering before selecting the top 'm' recommendations.
    recs = list(sorted(recommended))[:m]
